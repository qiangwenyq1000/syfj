...
public class DesensitizedUtils {
     ...
     * @return
     */
    private static boolean executeIsEffictiveMethod(Object javaBean, MaskingField desensitized) {
    ...
    public static String idCardNum(String id) {
        ...
        if(id.length()>12){
            return StringUtils.left(id, id.length()-12).concat(StringUtils.removeStart(StringUtils.leftPad(StringUtils.right(id, 4), StringUtils.length(id), "*"), "***"));
        }
        String num = StringUtils.right(id, 4);
        return StringUtils.leftPad(num, StringUtils.length(id), "*");
        //return StringUtils.left(id, 4).concat(StringUtils.removeStart(StringUtils.leftPad(StringUtils.right(id, 4), StringUtils.length(id), "*"), "******"));
