...
public class ValidateCodeUtil {
    ...
    private final String sessionKey = "RANDOMVALIDATECODEKEY";
    ...
    //生成随机图片
    public void getRandomCodeImage(HttpServletRequest request, HttpServletResponse response,RedisTemplate redisTemplate){
        ...
        //放缓存
        redisTemplate.opsForValue().set(randomStr.toLowerCase(),1,300, TimeUnit.SECONDS);
        try {
            //  将图片以png格式返回,返回的是图片
    ...
    //生成随机图片的base64编码字符串
    public String getRandomCodeBase64(HttpServletRequest request, HttpServletResponse response,RedisTemplate redisTemplate) {
        HttpSession session = request.getSession();
        // BufferedImage类是具有缓冲区的Image类,Image类是用于描述图像信息的类
        BufferedImage image = new BufferedImage(width, height, BufferedImage.TYPE_INT_BGR);
        Graphics g = image.getGraphics();
